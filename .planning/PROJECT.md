...
- [ ] v2.0 Public Beta — see Current Milestone above. Requirements to be scoped in `.planning/REQUIREMENTS.md`.
  ...
  - [x] CHLG-01..05 (Changelog: public feed, inline admin authoring, homepage hero, email subscribe/broadcast, unsubscribe) — Phase 9, validated 2026-08-07. All five live-proven against production: anonymous visitors see the post list with no composer; admin composer publishes inline and appears at the top instantly; homepage hero links to `/changelog`; subscribe is enumeration-safe (two identical POSTs → one row, byte-identical responses) and rate-limited (`200×5, 429` on the 6th); a real broadcast email was delivered to a human-checked inbox with subject/body/line-breaks intact and a working unsubscribe link, confirmed via DB row removal. Two small follow-ups shipped same-day at user request, outside the original plan scope: a Log Out nav entry, and admin post-editing (`PATCH /api/changelog/[id]`, no re-broadcast on edit) — both code-reviewed alongside the rest of the phase. A code-review finding (CR-01: `/api/unsubscribe` has no rate limiting, unlike its `/api/subscribe` and `/api/changelog` siblings — real availability concern given the VPS's 3.7GB/earlyoom memory ceiling) was assessed as not goal-blocking — none of the five CHLG success criteria depend on unsubscribe being rate-limited — and remains open for a follow-up fix.
...
---
*Last updated: 2026-08-07 — Phase 9 (Changelog) complete: CHLG-01..05 all validated live, human-confirmed real inbox delivery and unsubscribe round trip. CR-01 (no rate limit on /api/unsubscribe) open as non-blocking follow-up.*
