...
- [x] **Phase 15.2: Lighthouse final TBT reduction** (3/3 plans) — completed 2026-08-13 - The premise (revert qualities + split framework chunk) was investigated. The Phase 15.1 "regression" was VPS-load measurement noise — see `15.2-measurement-noise-evidence.md` for the disproof (identical framework chunk hashes, 28-byte script payload delta, benchmarkIndex 477.5 vs 1073). **What Phase 15.2 actually delivered:** (1) `scripts/lighthouse-bench.mjs` reusable median-of-N Lighthouse harness with ephemeral chrome, sequential execution for the 3.7GB VPS; (2) self-consistent logo + `next.config.ts` config; (3) `GlobalSearch` + `FeedbackModal` + `useSession()` deferral via `*Client.tsx` wrappers in `src/components/`; (4) `15.2-FINAL-ANALYSIS.md` — the corrected record with noise methodology. **Final clean measurement (PageSpeed Insights, Lighthouse v13.4.1, Google infrastructure):** Performance **96** / **100** / **100** on `/` / `/psalms` / `/psalms/23`. POLISH-03 flipped to `[x]` complete with the PSI evidence as the authoritative score (VPS-local Lighthouse was unreliable due to CPU contention). All earlier phases' code wins stand.

- [ ] **Phase 17: Vercel + Neon Migration** - Move psalter off the Hetzner VPS onto Vercel Hobby + Neon free tier, mirroring debates v10.0 (Phase 44). Plan 17-00 migrates `public/tunes/` (126MB) to Cloudflare R2 first; remaining plans provision Neon + Vercel, deploy + import data, cut over DNS (Cloudflare CNAME flips to Vercel), set up GH Actions cron if needed, and decommission the VPS footprint (Docker container, tunnel ingress, env vars). See `.planning/notes/migrate-to-vercel-neon.md` for the full decision + plan structure. **Depends on: nothing. Runs before Phase 6 and Phase 7 in execution order (the very next phase as of 2026-09-06).**
...
Phases 7 and 10 have no dependencies on Phase 6 or each other and may be worked in any order; Phase 8 and Phase 9 depend on Phase 7; Phase 11 depends on Phase 10; Phase 13 depends on Phase 6; Phase 14 depends on all prior v2.0 phases. **Phase 17 (Vercel + Neon Migration) is the next-to-execute phase as of 2026-09-06 — independent of Phase 6 and Phase 7, runs first.**
