...
- [x] **Phase 15.2: Lighthouse final TBT reduction** (3/3 plans) — completed 2026-08-13 - **The premise was investigated and disproved.** Phase 15.2 set out to revert `next.config.ts images.qualities` (believed to have caused the 15.1 regression) and split the framework runtime. Investigation in `.planning/phases/15.2-lighthouse-final-tbt-reduction/15.2-measurement-noise-evidence.md` showed the 15.1 regression was machine noise on a thrashing VPS run, not a bundle-graph change — the framework chunk hashes between Phase 15 and Phase 15.1 are identical (`0r5n9xaeyh.pb.js` in both) and the script payload differs by 28 bytes. **What Phase 15.2 actually delivered:** (1) `scripts/lighthouse-bench.mjs` — a median-of-N Lighthouse harness with ephemeral chrome, sequential execution to fit the 3.7GB VPS, supports `--runs N` and `--out path.json` flags; (2) `next.config.ts` self-consistent `qualities: [75]` matching runtime default; (3) `SiteHeader.tsx` `GlobalSearch`/`FeedbackModal` deferral via `next/dynamic({ ssr: false })` wrappers and `useSession()` consumption gated behind `requestIdleCallback` (3 new files: `GlobalSearchClient.tsx`, `FeedbackModalClient.tsx`, tests); (4) `15.2-FINAL-ANALYSIS.md` — the corrected record. **Median-of-5 final scores:** `/` 0.68 (spread 0.49–0.78), `/psalms` 0.69 (0.65–0.80), `/psalms/23` 0.45 (0.42–0.59). Each new median is inside its baseline spread — the plan's own noise rule rejects it as a real change. **Mechanical blocker for 0.90:** framework runtime `0r5n9xaeyh.pb.js` (71 KB, 910–1134 ms scripting on every route, 25 KB unused bytes that `next/dynamic` cannot split — unhit framework paths in React 19 + Next 16). **POLISH-03 stays `[ ] deferred`** with the user's three forward options recorded in `15.2-FINAL-ANALYSIS.md` (accept, RSC root layout refactor, or Cloudflare WAF rule).
