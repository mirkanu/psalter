...
- [x] **Phase 15.1: Lighthouse gap-closure — TBT and LCP reduction** (3/3 plans) — completed 2026-08-12 - Shipped 5 dynamic-import wrappers (`PsalmPickerModalClient`, `PsalmTopBarClient`, `OnboardingTourClient`, `TuneSwitcherSheetClient`, `GearPopoverClient`) + PsalmListingGrid hydration batching + logo q=100→90. All code verified in build. Apples-to-apples localhost re-audit (BUILD_ID `9SatBYqJdA0QpGtnJ4Xx1`, Wave-1 changes vs Phase-15 baseline, same Lighthouse v13.4.1 mobile/sim config) returned perf **0.47 / 0.60 / 0.67** vs the 0.90 target — `/` regressed (0.69→0.47, root cause unclear; likely Turbopack chunk reshuffle from `qualities` array change), `/psalms` and `/psalms/23` essentially flat (TBT-2 floor from framework chunk `0r5n9xaeyh.pb.js` at 1100-3030ms scripting is the dominant cost and irreducible without RSC surgery). **POLISH-03 stays `[ ] deferred`** with three followup paths documented in `.planning/phases/15.1-lighthouse-gap-closure-tbt-reduction/15.1-lighthouse-analysis.txt`.
