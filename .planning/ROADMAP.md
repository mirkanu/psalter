...
- [x] **Phase 15.1: Lighthouse gap-closure — TBT and LCP reduction** (3/3 plans) — completed 2026-08-12 - Shipped 5 dynamic-import wrappers (`PsalmPickerModalClient`, `PsalmTopBarClient`, `OnboardingTourClient`, `TuneSwitcherSheetClient`, `GearPopoverClient`) + PsalmListingGrid hydration batching + logo q=100→90. All code verified in build. Apples-to-apples localhost re-audit returned perf **0.47 / 0.60 / 0.67** vs the 0.90 target. **POLISH-03 stays `[ ] deferred`** — followup is Phase 15.2 (revert `qualities` array change that triggered `/` regression, split the framework-runtime chunk via more granular dynamic imports).
- [ ] **Phase 15.2: Lighthouse final TBT reduction** - Revert next.config.ts `qualities: [75,90]` to `[75]` (which alone recovered `/psalms` to 0.71 and `/` Parse-HTML-CSS from 2972ms → 139ms per round-2 A/B test). Then push further: split `0r5n9xaeyh.pb.js` framework-runtime chunk via granular `next/dynamic` on heavy client components (SingingView + OnboardingTour), and gate PsalmListingGrid's import in PsalmPickerModal behind a modal-open state. Realistic ceiling: 75-89 across all three routes; best case 90+.
