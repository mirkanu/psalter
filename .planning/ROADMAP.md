...
- [ ] **Phase 15.1: Lighthouse gap-closure — TBT and LCP reduction** - Close the real-Lighthouse gap on `/`, `/psalms`, `/psalms/[id]`. Top blockers from `bootup-time` / `mainthread-work-breakdown`: chunk `0r5n9xaeyh.pb.js` (70 KB, 30-35% unused, all routes), chunk `0uqgm3y7qfyr6.js` (3.2s script eval on /psalms), chunk `15bprl.pb34mm.js` (147 KB with 60 KB dead on psalm-detail).
