# Milestone v1.0: Public Psalter

**Status:** ✅ SHIPPED 2026-07-29
**Phases:** 1 – 4.12 (Milestone 1 scope; Phase 4.9/4.10/4.11/4.9.11/4.9.12 deferred — see Milestone Summary below)
**Total Plans:** 100+ plans across 26 phases (2 phases — 4.9, 4.9.11 — never executed; 4.9.12 partial; see Deferred section)

## Overview

Rebuild of psalter.cprc.co.uk (Airtable + Softr) into a self-hosted Next.js 15 + PostgreSQL application. Milestone 1 covers data migration through a full public-facing site: psalm browsing, tune pages, search, live abcjs notation with syllable-aligned staff view, and mobile-optimised singing view. Milestone 2 (Precentor Portal & Polish) continues separately in the live `.planning/ROADMAP.md`.
...
**Stats:**
- 1212 total commits across the project to date (Milestone 1 and Milestone 2 work interleaved chronologically — Phases 05.1–05.3 shipped 2026-06-15 to 2026-06-21, in the middle of Milestone 1's 4.9.x sequence, so git range/LOC below reflect the whole repo, not a clean M1-only slice)
- ~307,000 insertions / ~22,800 deletions across 2,954 file-changes (whole-project git history, 2026-05-07 → 2026-07-29)
- 29,278 LOC current TypeScript/TSX in `src/`
- Timeline: 2026-05-07 (Phase 1 start) → 2026-07-27 (Phase 04.9.15.1, last Milestone 1 phase to complete)

**Decimal Phases (INSERTED):**
- Phase 4.5–4.12: an extensive chain of inserted phases refining the psalm detail page, listing page, explore/tunes pages, and — the largest single thread — syllable-to-note staff alignment (4.9.6 through 4.9.10), which went through multiple hotfix cycles (4.9.7 fixed a dropped-metrical-lines regression from 4.9.6; 4.9.8 fixed word-cutoff at line ends; 4.9.9 added melisma/passing-note support; 4.9.10 re-ran phrase-break annotation to close out the alignment work at 150/150 psalms passing)
- Phase 04.10 and 04.11: two successive attempts at automating melisma/underline extraction from OCR — 04.10 proved the MusicXML-with-slurs pipeline on a single tune (Crimond) but didn't scale; 04.11 pivoted mid-execution from a mechanical batch OCR push to a manual `/dev/melisma-editor` workflow that is still in active use post-milestone-close

**Key accomplishments:**
1. Full Airtable → PostgreSQL migration with R2-hosted tune images (Phase 1), replacing a £20/month Airtable+Softr stack
2. Complete public browse experience: 150 psalms, tune pages, full-text search, topic/meter browse, 365-day reading plan (Phases 2–3)
3. Live abcjs SVG notation rendering with syllable-aligned lyrics under the staff, replacing static JPG-only tune display (Phase 4, 4.9.1–4.9.2)
4. Solved syllable-to-note staff alignment for the Scottish Psalter's CM/LM/SM/DCM/alternate-meter corpus — including DCM double-stanza pairing, mid-stanza Bible-verse boundaries, amen-ending suppression, and melisma/passing-note handling (Phase 4.9.6–4.9.10)
5. Mobile-first singing view: chromeless notation+lyrics view, onboarding tour, dynamic zoom, swipe stanza navigation, landscape-aware fit-to-screen (Phase 4.9.3–4.9.4, 4.9.15, 04.9.15.1)
6. Full /explore rebuild matching the reference site across 6 taxonomy sections — themes, NT quotations, messianic index, Nave's topics, authors, Heidelberg Catechism (Phase 4.12)

**Issues Resolved:**
- Contemplation tune "sharps play as naturals" audio bug — root-caused to abcjs accidental-scope bleeding across phrase-final music lines without a trailing barline; fixed and verified via MIDI pitch dump (debug session, closed at milestone archive)
- Mobile abc notation SVG overflow — root-caused to a missing `responsive: 'resize'` viewBox after an earlier commit removed it; fixed and verified live on production at mobile + desktop viewports (debug session, closed at milestone archive)
- DCM (double-stanza) tune mis-render, mid-stanza-verse-split, alternate-meter misalignment, and amen-ending bugs — fixed via the Phase 4.9.6 structured `Stanza→Line→Syllable` lyrics data model

**Issues Deferred (see `.planning/STATE.md` Deferred Items for full detail):**
- Phase 4.9 (Tune Notation Conversion) — superseded by the manual melisma-editor workflow before it was ever executed
- Phase 4.10 (Verified MusicXML Pilot) — 4/5 plans done, sing-test checkpoint never run, superseded by 4.11
- Phase 4.11 (Solfège Underline OCR) — pivoted to manual per-tune review; 70/172 tunes approved for live inline-Staff rendering at close, remainder gracefully fall back to JPEG scans
- Phase 4.9.11 (Lyric-Count False Positive Fix) — not started
- Phase 4.9.12 (Melisma positions as tune-level data) — 3/4 plans done; final NotationRenderer rewire not applied
- UAT/verification sign-off gaps on Phases 02, 04.5, 04.7, 04.9.8, 04.9.9, 04.9.14, 05, 05.1, 05.3 — features are live in production with no reported breakage, but the formal human-verify checkpoints were never closed out

**Technical Debt Incurred:**
- REQUIREMENTS.md traceability had drifted significantly stale relative to actual phase completion by the time of this milestone close (most Milestone-1 requirement checkboxes still read "Pending" despite the underlying phases being shipped weeks earlier) — corrected during archival; going forward, requirement checkboxes should be flipped at phase close, not deferred to milestone close
- 39 pre-2026-06 quick-task directories are flagged "missing" by the automated open-artifact audit despite being fully documented complete in STATE.md's Quick Tasks table — an audit-tooling format mismatch with the older quick-task convention, not real open work
- Melisma/notation OCR corpus is only 41% reviewed (70/172 tunes) at milestone close; this is intentionally treated as ongoing content curation rather than a release blocker
