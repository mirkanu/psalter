...
Last activity: 2026-07-16 - Round 5 on quick task 260715-s7b: user compared round 4's fix against known-good tunes (Ps 63/Wetherby) and flagged it was worse in a sense — St Agnes now showed 8 cramped systems with wasted side margins from the uniform transform-scale, vs Wetherby's clean 4. User approved the deeper investigation (asked question, got "full fix, verify broadly"). Root cause: a blanket rule forces every mobile phrase to split in half, but ONLY for tunes lacking DB melisma_positions — tunes with melisma data (Wetherby) bypass it entirely via a separate code branch and render 1 clean system per phrase already. Removed the forced split for chromeless<1280 (a design doc this rule cited actually specifies "4 systems" for CM, not 8). Verified broadly: sampled ~10 tunes across CM/LM/SM/other meters, with and without melisma data, via Playwright — all now render correctly with no scroll and no lost lyrics. Found and ruled out 2 unrelated pre-existing data bugs while sampling (confirmed present before this change too, via a stash-and-rebuild comparison) — out of scope for this task
