...
Last activity: 2026-07-16 - Round 4 on quick task 260715-s7b: after round 3's syllable-alignment fix, user reported a distinct issue — CM tunes should fit the whole notation on one mobile screen without scrolling (per ROADMAP Phase 4's stated goal), but the page still scrolled. Traced to the fit-to-viewport transform-scale mechanism built earlier for chromeless split-leaf (260712-szw) being gated to split-leaf only; inline (non-split) Staff — the default view — had no fit-scale at all despite a code comment nearby claiming it already worked. Broadened the trigger and added a distinct DOM marker so the existing split-leaf diagnostic script isn't affected. Verified via Playwright: scrollHeight===clientHeight on Ps 62 and Ps 1 at 375px; desktop confirmed unaffected via screenshot
...
| 260715-s7b | Fix inline Staff notation mobile issues for St Agnes, Durham (psalm 62) and other CM tunes, in 4 rounds: (1) mobile sub-divides phrases at measure boundaries then split syllables by equal token count, cramming syllables onto a lone held final note — fixed by weighting the split by each sub-staff's real note-head count; (2) still broken on real device — misdiagnosed forceMatchMeterShape's word-splitter as the cause and added detectRepeatedPitchContinuations, but this tune uses the solfege-driven buildWLineFromSolfa path which never reaches that code; (3) installed Playwright on the VPS (chromium binary + PLAYWRIGHT_BROWSERS_PATH mismatch with the daemon's default cache dir), instrumented the live page with a temp console.log of the exact ABC fed to abcjs, and found the real bug: buildWLineFromSolfa's Step-4 duration heuristic marks the shortest-duration note as a melisma "passing" continuation, tie-broken by earliest index — fixed by preferring detectRepeatedPitchContinuations' repeated-pitch-run positions before falling back to the duration heuristic; (4) separate issue — the whole tune required a page scroll on mobile even though ROADMAP Phase 4's goal says "all renders fit dynamically to screen"; the existing fit-to-viewport transform-scale mechanism (built for chromeless split-leaf in 260712-szw) was gated to split-leaf only, inline Staff had none — broadened `compactSplitMobile`'s trigger to cover inline Staff too, added a `data-notation-fit-slot` marker (kept distinct from split-leaf's `data-notation-slot` to not break the existing diagnostic script) | 2026-07-16 | 3a3f257 | [260715-s7b-fix-inline-staff-notation-layout-broken-](./quick/260715-s7b-fix-inline-staff-notation-layout-broken-/) — verified live via Playwright: DOM inspection confirms correct syllable-to-note mapping; mainScrollHeight===mainClientHeight confirms no scroll needed on Ps 62 and Ps 1 (CM); desktop screenshot-confirmed unaffected. Residual tight kerning between short adjacent syllables is cosmetic, not a misalignment |
