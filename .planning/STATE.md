...
Last activity: 2026-08-22 -- Completed 2 quick tasks in parallel worktrees: 260821-uqe (removed the duplicated Sing panel from /psalms/[id]/study, enriched the Study tab with per-verse Nave's Topics/Doctrine cross-references) and 260822-di9 (added a voluntary "View original scan" toggle to the singing view's gear popover, reusing AbcPlayer's existing showOriginal mechanism and fixing a real bug where the DB's score_jpg_url/solfege_jpg_url columns are always NULL) (master @ 15afaa1); previous quick task 260821-p2y (4 rounds): homepage layout/font fixes; Phase 15.2 closed previously; POLISH-03 flipped to `[x]` complete based on external Lighthouse measurement (Lighthouse v13.4.1, emulated Moto G Power, Slow 4G, run from Google's data centers — no VPS CPU contention)
...
| 260821-p2y | Fix 4 homepage issues found via Playwright review of the Softr rebuild: (1) `HomeCard` now always renders exactly one `Card` — when `children` are passed they render in a `CardContent` sibling inside the same Card instead of a stretched sibling `<div>` outside it; (2) `DailyTodayCard` stripped of its own `bg-muted`/`border-l-4` box chrome so it reads as part of the Daily HomeCard's single bordered surface; (3) new `ChangelogBanner` component — compact single-line "New: {latest post title} — Read the changelog" link above the h1, renders nothing with zero posts; (4) new `ExploreHighlights` component — 2x2 grid of Mood/Topic/Nave's Topics/Messianic shortcut links inside the Explore card, hrefs verified against `ExploreTabShell`'s actual `tab`/`sub` query-param values (not guessed). Follow-up round 1: added `items-start` to both homepage grid containers — without it, CSS grid's default `align-items:stretch` was still forcing shorter cards (Learn Tunes, Tunes Database) to match taller row siblings (Daily, Precenting Tool), reproducing the interior-void bug one level in. Follow-up round 2 (user-requested re-review): `items-start` still left lopsided gaps between rows since it only stops stretching, not repositioning — switched section 1's grid to a CSS multi-column masonry layout (`columns-1 md:columns-2` + `break-inside-avoid` wrapper divs) so cards pack into height-balanced columns (Choose Psalm+Explore stack in column 1, Daily+Learn Tunes in column 2) instead of sitting in rigid same-height rows; also shrank the Daily card's psalm-ref text from a raw `text-[28px]` down to `text-2xl` (24px) since it was visually outweighing the card's own 18px "Daily" title. Follow-up round 3 (second user re-review): switched the masonry grid's `gap-6` shorthand to `gap-x-6` (explicit `column-gap` longhand) for cross-browser multicol safety — the generic CSS `gap` shorthand's support for multi-column containers has a spottier browser history than the dedicated `column-gap` property. Also simplified the Daily homepage card from a full rich block (date, day badge, big psalm title, italic subtitle, "Sing psalm" button, via the shared `DailyTodayCard` component) down to one sentence — new `TodayPsalmBadge` component renders just the psalm ref as a small pill styled to match `ExploreHighlights`' font size (text-xs) and border chrome, appended inline to the existing description as "...Today's Psalm: [badge]". `HomeCard`'s `description` prop widened from `string` to `React.ReactNode` to allow embedding the badge inline. Daily no longer uses the `children` slot (badge is non-interactive, lives inside the card's own link, so no nested-anchor risk) — it's back to a plain single-link card like Choose Psalm/Learn Tunes. The shared `DailyTodayCard` component itself is untouched and still powers the full-featured `/daily` page unchanged. Follow-up round 4 (user reported via screenshot, dark mode): found and fixed the ACTUAL root cause of the missing vertical padding between stacked homepage cards — round 2's `last:[&>div]:mb-0` was intended to zero out only the last card's own trailing margin, but the masonry container itself is the last child of its `<section>`, so the selector matched the CONTAINER and zeroed `margin-bottom` on ALL its div children (confirmed via `getComputedStyle`: every wrapper reported `marginBottom: 0px` before the fix, `24px` after removing the selector) — light mode's subtle card-border contrast had been masking this since round 2, dark mode's near-identical card/page background colors made it obvious. Also reworked `TodayPsalmBadge` from inert text into a real clickable `Link` to `/psalms/{id}`, styled identically to the `ExploreHighlights` pills (border, rounded-md, text-xs, amber `Music2` icon) with a "Today's Psalm" caption above it on its own line — since it's now interactive, Daily reverted to using `HomeCard`'s `children` slot again (CardContent sibling of the card's own `/daily` link, avoiding a nested anchor). Verified fix in BOTH light and dark mode via Playwright (toggled the real theme button, not a forced DOM class, since `next-themes` re-render otherwise reset a manually-added class). Build/tsc/eslint/vitest clean (16/16 tests). | 2026-08-21 | 24d22ad | [260821-p2y-fix-four-homepage-layout-content-issues-](./quick/260821-p2y-fix-four-homepage-layout-content-issues-/) |
| 260821-uqe | Simplify psalm study view (`/psalms/[id]/study`): removed the duplicated lyrics+tune "Sing" panel from `PsalmTabs.tsx` (mobile Sing tab, always-visible desktop panel, and the dual stacked/sticky-2-column desktop layout that existed only to host it) — the canonical singing experience already lives one click away via the existing "Back to singing view" link to `/psalms/[id]` (confirmed byte-identical afterward via `git diff --stat`). Desktop study view collapsed to one single-column tabbed area (Overview/365 Days/Study/Messianic/Parallel), mobile now defaults to Overview. Enriched the Study tab with a new "Cross-References" section showing per-verse Nave's Topics and Doctrine references — data `fetchPsalmDetail` already fetched (`verses[].verseNavesTopics`/`verseDoctrines`) but that went completely unused anywhere in the file; mapped topics link to `/explore/naves/{slug}` via the existing `buildNavesSlugMap` helper, unmapped ones render as inert badges. 339 lines removed from `PsalmTabs.tsx`, net -199 lines across the change. | 2026-08-22 | fcaa584 | [260821-uqe-simplify-psalm-study-view-psalms-id-stud](./quick/260821-uqe-simplify-psalm-study-view-psalms-id-stud/) |
| 260822-di9 | Add a voluntary "View original scan" toggle to the singing view's `GearPopover` (shared by `/psalms/[id]` and `/precent/[id]/sing/[pos]` via `SingingView.tsx`), letting users swap the live abcjs digital rendering for the scanned JPG and back without a page reload. Reused `AbcPlayer`'s existing (but previously unreachable — hidden behind `hidePlayerControls` in fullscreen/chromeless mode) `showOriginal` mechanism: lifted it out of `NotationRenderer.tsx`'s internal state into a controlled prop mirroring the exact pattern `viewMode` already used, then threaded it up through `SingingView.tsx` as sibling state (same lifting pattern `viewMode` uses to reach both `NotationRendererClient` and `GearPopoverClient`) and exposed it as a new "Score: Digital / Original scan" row in the gear popover, deliberately kept separate from `viewMode`'s existing `staff-split`/`solfege-split` states so this voluntary toggle can never route a user back into an inline rendering the MOBILE-08 approval gate is intentionally blocking. Bug found+fixed during implementation: `tunes.score_jpg_url`/`solfege_jpg_url` DB columns are NULL for every tune — the real scan URLs are server-derived via `deriveTuneJpgPages` (filesystem scan), so the underlying swap could never have worked for any tune before this fix regardless of UI; `originalScanAvailable` now falls back to the derived staff/solfège page arrays. `/precent/[id]/sing/[pos]` gets the fix for free since it renders the same `SingingView`. Verified live via a purpose-built Playwright diagnostic (`tests/diagnostics/original-scan-toggle.mjs`, 11/11 steps pass): score row visible, click swaps SVG→JPG with no navigation, both return paths (gear "Digital" radio and the existing `BackToNotationButton`) restore the live SVG. Explicitly out of scope: the `/tunes/[slug]` tune page (different Staff/Solfège routing logic, separate follow-up). | 2026-08-22 | 15afaa1 | [260822-di9-add-a-voluntary-view-original-scan-toggl](./quick/260822-di9-add-a-voluntary-view-original-scan-toggl/) |
