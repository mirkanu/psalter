...
Last activity: 2026-07-16 - Round 3 on quick task 260715-s7b: user asked me to set up Playwright and self-verify instead of guessing from their screenshots. Installed Chromium for the shared playwright-daemon (found + fixed a broken /home/claude/.cache symlink and a PLAYWRIGHT_BROWSERS_PATH env var the PM2 daemon process didn't inherit). Used it to instrument the live page and found round 2's diagnosis was wrong — St Agnes uses the solfege-driven buildWLineFromSolfa path, not the syllabifyForAbc path round 2 patched. The real bug: buildWLineFromSolfa's duration heuristic picks the shortest-duration note as a melisma continuation, tie-broken by earliest index, so it picked the FIRST of three tied-duration repeated notes instead of a later one. Fixed by preferring detectRepeatedPitchContinuations' output there. Verified correct via direct Playwright DOM inspection of the rendered SVG (not just screenshots) — syllables now land on the right notes
...
| 260715-s7b | Fix inline Staff notation mobile lyric misalignment for held-final-note tunes (e.g. St Agnes, Durham / psalm 62), in 3 rounds: (1) mobile sub-divides phrases at measure boundaries then split syllables by equal token count, cramming syllables onto a lone held final note — fixed by weighting the split by each sub-staff's real note-head count; (2) still broken on real device — misdiagnosed forceMatchMeterShape's word-splitter as the cause and added detectRepeatedPitchContinuations, but this tune uses the solfege-driven buildWLineFromSolfa path which never reaches that code; (3) installed Playwright on the VPS (chromium binary + PLAYWRIGHT_BROWSERS_PATH mismatch with the daemon's default cache dir), instrumented the live page with a temp console.log of the exact ABC fed to abcjs, and found the real bug: buildWLineFromSolfa's Step-4 duration heuristic marks the shortest-duration note as a melisma "passing" continuation, tie-broken by earliest index — St Agnes' phrase 1 opens with 3 tied-duration repeated notes and it picked note 0 (needs its own syllable) instead of a later repeat; fixed by having it prefer detectRepeatedPitchContinuations' repeated-pitch-run positions before falling back to the duration heuristic. Verified via Playwright DOM inspection of the actual abcjs-lyric SVG text elements (not just a screenshot) — syllables now land on the correct notes | 2026-07-16 | 53748e9 | [260715-s7b-fix-inline-staff-notation-layout-broken-](./quick/260715-s7b-fix-inline-staff-notation-layout-broken-/) — verified live via Playwright DOM inspection; residual tight kerning between short adjacent syllables (e.g. "soul"/"with" touching) is cosmetic, not a misalignment |
