...
Last activity: 2026-07-16 - Quick task 260715-s7b CONFIRMED FIXED by user on their own mobile device, closing out a 5-round investigation (mobile Staff notation for St Agnes/Ps 62 and CM tunes generally): syllable-to-note misalignment (fixed via note-count-weighted splitting, then via correcting buildWLineFromSolfa's passing-note heuristic), missing viewport-fit scaling, and finally an over-aggressive forced-mobile-phrase-split rule that was doubling CM tunes to 8 systems instead of the intended 4. SUMMARY.md rewritten to document the full 5-round journey; two unrelated pre-existing data bugs found while broadly verifying (tune 12 truncated ABC, tune 136 lost-lyrics-after-phrase-1) are logged as out-of-scope follow-ups, not fixed here
