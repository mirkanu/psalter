...
Last activity: 2026-07-15 - Round 2 on quick task 260715-s7b: after deploy, user re-tested on real mobile device and it was still messed up. Root-caused via live pipeline testing (ran the actual syllabifyForAbc/forceMatchMeterShape/splitWLineByNoteCounts functions against St Agnes' real data) to a deeper pre-existing bug — forceMatchMeterShape invents a fake mid-word split ("soul"→"sou-"+"l") to pad a naturally-7-syllable line to CM's expected 8 syllables, which collides visually once packed into mobile's narrow sub-staff. Added detectRepeatedPitchContinuations to auto-derive melisma continuations from repeated-pitch note runs (the "reciting note" convention) before falling back to word-splitting. Deployed and live; awaiting user re-confirmation on their phone
...
| 260715-s7b | Fix inline Staff notation mobile lyric misalignment for held-final-note tunes (e.g. St Agnes, Durham / psalm 62), in 2 rounds: (1) mobile sub-divides phrases at measure boundaries then split syllables by equal token count, cramming syllables onto a lone held final note — fixed by weighting the split by each sub-staff's real note-head count; (2) deployed + user re-tested on real mobile device, still messed up — root-caused via live pipeline testing to forceMatchMeterShape fabricating a fake mid-word split ("soul"→"sou-"+"l") to pad a 7-syllable line to CM's expected 8, which visually collided with neighbouring text once packed into a narrow mobile sub-staff; fixed by detectRepeatedPitchContinuations auto-deriving melisma continuations from repeated-pitch note runs (the psalm "reciting note" convention, lyric-to-note-alignment.md §4) before falling back to the word-split heuristic | 2026-07-15 | 0d41763 | [260715-s7b-fix-inline-staff-notation-layout-broken-](./quick/260715-s7b-fix-inline-staff-notation-layout-broken-/) — unit tests/tsc/build all pass, deployed live at psalter.gsdlabs.dev; awaiting user re-confirmation on real mobile device |
