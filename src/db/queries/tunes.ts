...
/**
 * Lite enrichment for AlternateTune[] produced by fetchTunesByMeter.
 *
 * Original enrichAlternateTunesToTuneRows relies on fetchAllTunes() (every tune
 * in the DB joined with moods + psalmVersionTunes). For per-meter enrichment of
 * a small set of ids (≤ ~50), that does 3 round trips on a hot RSC path even
 * with React cache(), and it drags in tuneMoods + psalmVersionTunes rows for
 * ALL ~700 tunes we don't need.
 *
 * fetchTunesByMeterEnriched runs the three lookups we actually need in three
 * targeted inArray(...) queries and merges them into the AlternateTune shape:
 *   - tuneMoods -> moods[].name (for the TunePicker "moods" column / filter)
 *   - psalmVersionTunes -> psalmVersion.psalmId -> unique psalm ids
 *     (for recommendedPsalmIds + the default sort)
 *   - inPrcaPsalter, hasFamousHymn, famousHymn, numberIn1979RpPsalter,
 *     numInPrcaPsalter (for the In-PRCA / Famous-Hymn filter columns)
 *
 * Issue #72: the full-fat version was the main contributor to the 50-100 MB
 * ISR/lambda payload on cold starts. The lite version only fetches the rows
 * for the meter(s) the user is viewing.
 */
export async function fetchTunesByMeterEnriched(meter: string): Promise<EnrichedTuneRow[]> {
  const base = await fetchTunesByMeter(meter)
  if (base.length === 0) return []
  const ids = base.map((t) => t.id)

  // Single join: pulls only the columns the picker actually displays.
  const rows = await db.query.tunes.findMany({
    ...
    columns: {
      ...
      hasFamousHymn: true,
      famousHymn: true,
      numberIn1979RpPsalter: true,
      numInPrcaPsalter: true,
    },
    with: {
  ...
  return base
    .map((t) => {
      ...
      return {
        ...t,
        inPrcaPsalter: r.inPrcaPsalter ?? false,
        hasFamousHymn: r.hasFamousHymn ?? false,
        famousHymn: r.famousHymn,
        numberIn1979RpPsalter: r.numberIn1979RpPsalter,
        numInPrcaPsalter: r.numInPrcaPsalter,
        moods: r.tuneMoods.map((tm) => tm.mood.name).filter(Boolean) as string[],
        recommendedPsalmIds,
      } satisfies EnrichedTuneRow
    })
    .filter((x): x is EnrichedTuneRow => x !== null)
}
