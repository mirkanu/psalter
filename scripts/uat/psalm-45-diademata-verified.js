// Wave 2 UAT for Diademata (tune id 134, SM, psalm 45) — runs AFTER Task 4 build+restart.
...
//
// Test path: /tunes/diademata → click "Notation" tab → abcjs SVG should render.
//
...
async function verifyOnDevice(daemonUrl, deviceName, label) {
  ...
  const script = `
    ...
    try {
      await p.goto('${UAT_URL}/tunes/diademata', { waitUntil: 'networkidle', timeout: 30000 });
      ...
      if (!clicked) {
        return { ok: false, reason: 'no Notation tab found on /tunes/diademata' };
      }
      ...
      await p.screenshot({ path: 'scripts/uat/screenshots/psalm-45-diademata-verified-${label}.png', fullPage: false });
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      // Diademata SM has 26 syllables + 1 melisma continuation = 27 note groups.
      ...
      // Diademata SM has 4 phrases → 4 staff systems.
      ...
      if (!/diademata/i.test(result.pageH1 || '')) {
        const msg = `tune page h1 missing "Diademata" — got: "${result.pageH1}"`
        console.error(`FAIL [${vp.label}]: ${msg}`)
        if (vp.primary) primaryErrors.push(`${vp.label}: ${msg}`)
        else secondaryErrors.push(`${vp.label}: ${msg}`)
        continue
  ...
  if (secondaryErrors.length) {
    ...
    console.log(`PASS — Diademata abc_notation renders on iPhone13 and Pixel5`)
  }
  process.exit(0)
}
